...
function session(id, topic = "Holdstation Mini App") { return { id: `session_${id}`, appId: "holdstation-mini-app", appName: "Holdstation Mini App", topic, messages: [], contextUsed: [], missingContext: [], status: "completed", createdAt: "2026-05-25T17:00:00Z", updatedAt: "2026-05-25T17:00:00Z", isDevelopmentFallback: false, isRuntimeFallback: false, runtimeStatus: "live", runtimeMode: "live", contextQualitySummary: { selectedCount: 0, existingCount: 0, missingCount: 0, confirmedCount: 0, draftCount: 0, placeholderCount: 0 }, assumptions: [], suggestedActions: [], savedToVault: false }; }
async function capture(kind, message, answer, runtimeLabel = "", runtimeAgent = "cmo", extras = {}) { return autoCaptureTurnOnce({ request: request(message), session: session(kind), assistantMessageId: `msg_${kind}`, sourceUserMessageId: `user_${kind}`, answer, routeKind: "app-chat-response", runtimeSource: "dashboard", assistantFooterSourceLabel: runtimeLabel, runtimeLabel, runtimeProvider: "dashboard", runtimeAgent, ...extras }); }
...
const fanoutEcho = await autoCaptureTurnOnce({ request: fanoutRequest, session: fanoutSession, assistantMessageId: "msg_fanout_echo", sourceUserMessageId: "user_fanout", answer: browserLikeEchoAnswer, routeKind: "app-chat-response", runtimeSource: "dashboard", runtimeLabel: "Live - echo", runtimeProvider: "dashboard", runtimeAgent: "echo" });
...
assert.match(text, /auto-capture/); assert.match(text, /raw-capture/); assert.doesNotMatch(text, /capture-preview/); assert.match(text, /capture_origin: "auto"/); assert.match(text, /gbrain_status: "pending"/);
...
const dup = await autoCaptureTurnOnce({ request: request("/echo Write changed X post"), session: session("echo"), assistantMessageId: "msg_echo", answer: "changed content", runtimeLabel: "Live - echo", runtimeProvider: "dashboard", runtimeAgent: "echo" });
assert.equal(dup.duplicate, true);
assert.equal(echo.relativePath, dup.relativePath);
const preview = buildManualCapturePreview({ eventType: "last30days_trend", content: "Draft 1", appId: "holdstation-mini-app" });
