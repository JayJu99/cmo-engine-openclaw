...
try {
  ...
  try {
    ...
    assert.match(source, /function completedUnifiedCmoAgentPersistStateFromHermesResult/);
    assert.match(source, /routeKind !== "cmo_agent" && routeKind !== "creative_execution" && creativeArtifacts\.length === 0/);
    assert.match(source, /const earlyCompletedUnifiedCmoAgentPersistState = !creativeContractViolation/);
    assert.match(source, /if \(earlyCompletedUnifiedCmoAgentPersistState\) \{[\s\S]*usedHermesCmoChat = true;/);
    assert.match(source, /if \(!counterValidation\.ok && !earlyCompletedUnifiedCmoAgentPersistState\)/);
    assert.match(source, /completedUnifiedCmoAgentAnswer = answer/);
    assert.match(source, /rawHermesAnswer: hermesAnswerTextForFinalProjection\(result\.response\.answer\)/);
    assert.match(source, /normalizedHermesAnswer: normalizedAnswer/);
    assert.match(source, /completedUnifiedCmoAgentAnswerBasisMode = stringValue\(unifiedAnswerBasis\.mode, "cmo_agent"\)/);
    assert.match(source, /userVisibleAnswerPathLike\(normalizedAnswer\)/);
